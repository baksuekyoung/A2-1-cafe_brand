...
## 보너스 — 다국어 네이밍 지원 (2번 선택)

> 명세의 보너스 두 가지 중 **2번** 을 택했습니다.
...
**영문 표기를 짓는 규칙** — 간판·도메인·SNS 계정에 그대로 쓸 수 있어야 합니다.

- 소리 나는 대로 옮기거나(온기 → Ongi), 뜻이 통하는 영어 낱말로(쉼표 → Comma)
- 알파벳만, 12자 안쪽. 숫자·기호·띄어쓰기 없음
- 영어권 사람이 발음하기 쉽고, 다른 뜻으로 읽히지 않을 것
- 후보끼리 영문 표기가 겹치지 않을 것

**읽는 법은 영문 표기를 그대로 베끼지 않습니다.** 한국어 사용자가 소리 내어 읽도록
음절을 하이픈으로 나눕니다 (`Ongi → OWN-gee`). 베껴 오면 `run_report.md`에 기록됩니다.

셋 중 하나라도 비거나 규격에 어긋나면 리포트에 남습니다. **버리지는 않습니다** —
채택은 사람이 판단할 문제입니다.
...
| **보너스 2번 · 다국어 네이밍 지원** | ✅ **← 우리가 택한 것** | 후보마다 한글 + 영문 표기 + 읽는 법 |
| 보너스 1번 · 경쟁사 분석 | ✅ 덤으로 구현 | 택하지 않았지만 동작합니다 |
