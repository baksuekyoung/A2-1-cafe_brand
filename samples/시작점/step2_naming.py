...
박연수님이 `#2단계` 채널에서 이렇게 적으셨습니다.

> "자료 정리는 했으나 코드 연결은 앞 단계 확정이 안된 상태이기도 했고
>  제가 아직 해당 과정이 미숙하여 실질적인 연결은 진행을 못했습니다.
>  ... 1단계&5단계 담당 분들이 최종 연결해 주셔도 괜찮습니다"

그래서 **연결은 [5] 담당(노희선)이 붙였습니다.** 아래 코드는 그대로 돌아갑니다.
프롬프트 내용은 박연수님의 `국내_카페_브랜드_BI_분석_리포트` 를 그대로 옮긴 것이라
고치실 곳이 있다면 `NAMING_RULE` · `SLOGAN_RULE` · `STORY_RULE` 세 군데면 됩니다.

## 어떻게 도는가

    .env 에 OPENAI_API_KEY 가 있으면  →  진짜 LLM 호출
    없거나 호출이 실패하면          →  EXAMPLE 로 대신 돌린다 (파이프라인은 안 멈춤)

두 번째가 중요합니다. 발표 당일 API 키가 막히거나 쿼터가 끊겨도
전체 실행이 죽지 않습니다. 무엇을 썼는지는 `run_report.md` 에 남습니다.
"""
